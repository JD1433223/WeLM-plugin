...
class Setting {
  constructor () {
    ...
    /** 用户设置 */
    this.configPath = `${_path}/plugins/WeLM-plugin/config/`
    this.config = {}

    this.dataPath = `${_path}/plugins/WeLM-plugin/data/`
    this.data = {}
  ...
  getFilePath (app, type) {
    if (type === 'defSet') return `${this.defPath}${app}.yaml`
      ...
      try {
        ...
        logger.error(`WeLM插件缺失默认文件[${app}]${error}`)
      }
  ...
  watch (file, app, type = 'defSet') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[WeLM插件][修改配置文件][${type}][${app}]`)
      if (this[`change_${app}`]) {
        this[`change_${app}`]()
      }
    })
